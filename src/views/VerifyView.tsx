...
interface Props {
    client: PluginApi<Readonly<IRemixApi>> & PluginClient<Api, Readonly<IRemixApi>>
    apiKey: string
}
...
export const VerifyView: React.FC<Props> = ({ apiKey, client }) => {
    ...
    const onVerifyContract = async (values: FormValues) => { // TODO
        ...
        if (!compilationResult)
            throw new Error('no compilation result available') // TODO handle better, create a logger and a submit issue button
        ...
        const verify = async (apiKey: string, contractAddress: string, contractArguments: string, contractName: string, compilationResult: any) => {
            const network = await getNetworkName()

            const fileName = compilationResult.source.target // Check if it is not empty

            console.log("Filename", fileName)

            const contractMetadata = compilationResult.data.contracts[fileName][contractName]['metadata']
            ...
            const data: { [key: string]: string | any } = {
                ...
                optimizationUsed: contractMetadataParsed.settings.optimizer.enabled ? 1 : 0, //0 = Optimization used, 1 = No Optimization
        ...
        const result = await verify(apiKey, values.contractAddress, contractArguments, values.contractName, compilationResult)
    }
            ...
            <Formik
                ...
                {({ errors, touched, handleSubmit }) => (
                    <form onSubmit={handleSubmit}>
                        ...
                        <button type="submit" className="btn btn-secondary" style={{ padding: ".175rem 0.6rem" }}>Verify Contract</button>
                    </form>
                )}
            </Formik>
